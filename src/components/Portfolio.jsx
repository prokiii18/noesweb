...
function ProjectDialog({ project, open, onClose }) {
  ...
  return (
    <dialog ref={dialogRef} aria-labelledby="project-dialog-title" onClick={(event) => { if (event.target === dialogRef.current) dialogRef.current?.close(); }}>
      <button className="dialog-close" type="button" aria-label="Zavřít detail realizace" onClick={() => dialogRef.current?.close()}>×</button>
      <img className="dialog-image" src={project.image} alt={`${project.name} — detail realizace`} />
      <div className="dialog-copy"><span className="dialog-tags">{project.type}</span><h2 id="project-dialog-title">{project.name}</h2><p className="dialog-description">{project.description}</p></div>
    </dialog>
  );
}
...
export default function Portfolio() {
  ...
  const measure = useCallback(() => {
    const eligible = window.matchMedia('(min-width: 850px) and (min-height: 720px) and (prefers-reduced-motion: no-preference)').matches;
  ...
  const paint = useCallback(() => {
    ...
    const progress = Math.max(0, Math.min(1, (20 - sectionRef.current.getBoundingClientRect().top) / travelRef.current));
    select(Math.round(progress * (projects.length - 1)));
  }, [scrollMode, select]);

  useEffect(() => {
    measure();
    const media = window.matchMedia('(min-width: 850px) and (min-height: 720px) and (prefers-reduced-motion: no-preference)');
  ...
  const go = (index) => {
    ...
    if (scrollMode && travelRef.current && sectionRef.current) {
      ...
      window.scrollTo({ top: start + (travelRef.current * next) / (projects.length - 1), behavior: 'smooth' });
    } else {
      select(next);
    }
  };
  ...
  const openDialog = (event) => {
    ...
    setDialogProject(projects[current]);
  };
  ...
  return (
    <>
      <section id="projects" ref={sectionRef} className={scrollMode ? 'section projects scroll-cases' : 'section projects'} style={railHeight ? { '--case-rail-height': `${railHeight}px` } : undefined}>
        <div className="portfolio-pin" ref={pinRef}>
          <div className="portfolio-heading"><div><span className="section-label">02 / Naše práce</span><h2>Vybrané realizace.</h2></div><p>{scrollMode ? 'Scrollujte a objevte jednotlivé realizace.' : 'Vyberte značku a prohlédněte si realizaci.'}</p></div>
          <div className="case-selector" role="group" aria-label="Vybrat realizaci">
            {projects.map((item, index) => (
              <button key={item.name} type="button" aria-pressed={index === current} onClick={() => go(index)} onKeyDown={(event) => {
                if (!['ArrowRight', 'ArrowLeft'].includes(event.key)) return;
                event.preventDefault();
                const next = index + (event.key === 'ArrowRight' ? 1 : -1);
                go(next);
                const buttons = event.currentTarget.parentElement?.querySelectorAll('button');
                buttons?.[(next + projects.length) % projects.length]?.focus({ preventScroll: true });
              }}><span>{String(index + 1).padStart(2, '0')}</span>{item.name}</button>
            ))}
          </div>
